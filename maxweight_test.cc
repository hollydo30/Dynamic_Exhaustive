...
int main()
{
	...
	rubric.criterion(
		...
		[&]()
		{
			...
			std::cout<<calorie_small<<std::endl;
			std::cout<<weight_small<<std::endl;
			std::cout<<calorie_large<<std::endl;
			std::cout<<weight_large<<std::endl;
...
int main ()
{
	...
	//f.push_back(std::shared_ptr<FoodItem>(new FoodItem("a", 4.0, 5.0)));
	f.push_back(std::shared_ptr<FoodItem>(new FoodItem("b", 1.0, 5.5)));
	f.push_back(std::shared_ptr<FoodItem>(new FoodItem("c", 5.0, 1.2)));
	f.push_back(std::shared_ptr<FoodItem>(new FoodItem("d", 10.0, 4.7)));
	
	//f.push_back(std::shared_ptr<FoodItem>(new FoodItem("e", 1.0, 2.5)));
	//f.push_back(std::shared_ptr<FoodItem>(new FoodItem("f", 2.0, 2.9)));
	//f.push_back(std::shared_ptr<FoodItem>(new FoodItem("g", 3.0, 1.7)));
	//f.push_back(std::shared_ptr<FoodItem>(new FoodItem("h", 4.0, 1.0)));
	
	
	//auto filter = filter_food_vector(f, 0, 10, 8);
	
	//print_food_vector(*filter);
	
	auto dynamic = dynamic_max_weight(f, 3);
	
	print_food_vector(*dynamic);
	
	auto ex = exhaustive_max_weight(f, 3);
